...
class BlockEntityManager {
    ...
    registerEntityAsMachine(entity: Entity): void {
        if (this.activeMachineInstances.has(entity.id)) return;

        let blockLocationStr = entity.getDynamicProperty('blockLocation') as string | undefined;
        let blockLocation: Vector3 | undefined;

        // Determine blockId
        let blockId = entity.getDynamicProperty('machineId') as string | undefined;
    ...
    removeMachine(entityId: string, machineInstance: MachineInstance): void {
    ...
    handlePlayerViewCheck(): void {
        ...
        const machinesToShrink = new Set<string>();
        ...
        for (const player of world.getAllPlayers()) {
            ...
            let targetMachine: MachineInstance | null = null;
                    ...
                    if ((system as any).currentTick - this.lastPlacementTick > 20) {
                        ...
                        try {
                            ...
                            if (!this.activeMachineInstances.has(entity.id)) {
                                const machineInstance = new MachineClass(entity, blockHit.block) as MachineInstance;
                                machineInstance.locKey = locKey;
        ...
        while (processedCount < PROCESS_LIMIT && attempts < totalMachines) {
            ...
            try {
                ...
                if (dt > 0) {
                     ...
                     let currentBlockTypeId: string | undefined;
                     try {
    ...
    getXP(blockLocation: Vector3): number {
        let xp = 0;
