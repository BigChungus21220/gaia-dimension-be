import { system, BlockPermutation, GameMode, ItemStack, world, Block, Dimension, Player, BlockComponentRegistry, PlayerInteractWithBlockBeforeEvent, PlayerPlaceBlockAfterEvent, PlayerBreakBlockBeforeEvent, Vector3 } from "@minecraft/server";
...
const activeCurtains: CurtainInfo[] = [];

const relativeDirs: Record<string, { left: string, right: string }> = {
...
function initializeCurtainSystem(): void {
    system.runInterval(() => {
...
function updateCustomCurtainsFromLever(leverBlock: Block): void {
    const isLeverOn = leverBlock.permutation.getState("open_bit" as any);
    const newState = isLeverOn as boolean; 
                    ...
                    if (checkBlock && !checkBlock.isAir) {
                        if (checkBlock.typeId.includes("gaiadimension:") && (checkBlock.typeId.includes("curtain") || checkBlock.typeId.includes("door"))) {
                            ...
                            if (perm.getState("gaiadimension:open" as any) !== undefined) {
                                checkBlock.setPermutation(perm.withState("gaiadimension:open" as any, newState));
...
function toggleCustomCurtain(curtainBlock: Block, player: Player): void {
    ...
    const openState = perm.getState("gaiadimension:open" as any);
    if (openState === undefined) return;

    const newState = !openState;
    curtainBlock.setPermutation(perm.withState("gaiadimension:open" as any, newState));
    ...
    if (otherBlock && otherBlock.typeId === expectedOtherTypeId) {
        ...
        if (otherPerm.getState("gaiadimension:open" as any) !== undefined) {
            otherBlock.setPermutation(otherPerm.withState("gaiadimension:open" as any, newState));
        }
    }
}

function toggleTrapdoor(block: Block, player: Player): void {
    const isOpen = block.permutation.getState("gaiadimension:open" as any);
    const newState = !isOpen;
    block.setPermutation(block.permutation.withState("gaiadimension:open" as any, newState));
    player.playSound(newState ? "open.wooden_trapdoor" : "close.wooden_trapdoor", { location: block.location, volume: 1, pitch: 1 });
}

function isValidNeighbor(neighbor: Block | undefined, typeId: string, rotation: string | number | boolean | undefined): boolean {
    return !!(neighbor && neighbor.typeId === typeId && 
           neighbor.permutation.getState("minecraft:cardinal_direction" as any) === rotation);
}

function updateToDouble(block: Block, side: "left" | "right"): void { // side: "left" or "right"
    system.run(() => {
     try {
         ...
         const open = perm.getState("gaiadimension:open" as any);
         const facing = perm.getState("minecraft:cardinal_direction" as any);
         
         // Update Bottom
         block.setType(newTypeId);
         const newPerm = block.permutation.withState("gaiadimension:open" as any, open).withState("minecraft:cardinal_direction" as any, facing);
         ...
         if (topBlock) {
             ...
             const newTopPerm = topBlock.permutation.withState("gaiadimension:open" as any, open).withState("minecraft:cardinal_direction" as any, facing);
...
function updateActiveCurtainType(location: Vector3, newTypeId: string): void {
...
function destroyPartner(block: Block, dimension: Dimension): void {
    system.run(() => {
    try {
        ...
        removeFromActiveCurtains(block.location);
         ...
         const isValidVertical = otherVertical && (isBottom 
...
function removeFromActiveCurtains(location: Vector3): void {
...
export function registerCurtainComponent({ blockComponentRegistry }: { blockComponentRegistry: BlockComponentRegistry }): void {
    blockComponentRegistry.registerCustomComponent("gaiadimension:curtain", {});

    initializeCurtainSystem();

    registerPlaceHandler({
        check: (block: Block) => block.typeId.includes("_lower") || block.typeId.includes("_bottom"),
        execute: (event: PlayerPlaceBlockAfterEvent) => {
            ...
            if (blockAbove?.isAir) {
                ...
                const rotation = lowerPerm.getState("minecraft:cardinal_direction" as any);
                ...
                try {
                    ...
                    if ((block.typeId.includes("_bottom") || block.typeId.includes("_lower")) && 
                        ...
                        const dirs = (relativeDirs as any)[rotation as string];
                        
                        if (dirs) {
                            // Try Right Neighbor (We become Left, They become Right)
                            const rightNeighbor = (block as any)[dirs.right]();
                            if (isValidNeighbor(rightNeighbor, block.typeId, rotation)) {
                                 ...
                                 const leftNeighbor = (block as any)[dirs.left]();
                    ...
                    for (const dir of ["north", "south", "east", "west"]) {
                        const neighbor = (block as any)[dir]();
                        if (neighbor?.typeId === block.typeId && neighbor.permutation.getState("minecraft:cardinal_direction" as any) === rotation) {
                            if (neighbor.permutation.getState("gaiadimension:inverse" as any) === false) {
                                block.setPermutation(block.permutation.withState("gaiadimension:inverse" as any, true));
                                blockAbove.setPermutation(blockAbove.permutation.withState("gaiadimension:inverse" as any, true));
                                break;
                            }
                        }
                    }
    ...
    registerBreakHandler({
        event: "before",
        check: (block: Block) => block.typeId.includes('curtain') || block.typeId.includes('door'),
        execute: (event: PlayerBreakBlockBeforeEvent) => {
            ...
            if (brokenBlockTypeId.includes("_left") || brokenBlockTypeId.includes("_right")) {
                const rotation = block.permutation.getState("minecraft:cardinal_direction" as any);
                const dirs = (relativeDirs as any)[rotation as string];
                const isLeft = brokenBlockTypeId.includes("_left");
                
                if (dirs) {
                    const partnerDir = isLeft ? dirs.right : dirs.left;
                    const partner = (block as any)[partnerDir]();
                    ...
                    if (partner && partner.typeId === expectedPartnerType && 
                        partner.permutation.getState("minecraft:cardinal_direction" as any) === rotation) {
    ...
    registerInteractHandler({
        check: (block: Block) => block.typeId.includes("curtain") || block.typeId.includes("door") || block.typeId.includes("trapdoor"),
    ...
    registerInteractHandler({
        check: (block: Block) => block.typeId.startsWith("minecraft:") && block.typeId.includes("lever"),
        execute: (event: PlayerInteractWithBlockBeforeEvent) => {
            const { block } = event;
            system.run(() => {
