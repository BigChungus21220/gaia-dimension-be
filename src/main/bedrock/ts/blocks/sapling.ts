import { world, GameMode, system, Block, ItemStack, Player, EntityEquippableComponent, BlockComponentRegistry } from "@minecraft/server";
...
export function registerSaplingComponent({ blockComponentRegistry }: { blockComponentRegistry: BlockComponentRegistry }) {
    ...
    registerPlaceHandler({
        check: (block: Block) => block.typeId in saplingConfig,
        execute: (event: any) => {
             const { block } = event;
             ...
             system.run(() => {
                ...
                const config = (saplingConfig as any)[block.typeId];
    ...
    registerInteractHandler({
        ...
        execute: (event: any) => {
            system.run(() => {
                const { block, player, itemStack } = event as { block: Block, player: Player, itemStack: ItemStack };
