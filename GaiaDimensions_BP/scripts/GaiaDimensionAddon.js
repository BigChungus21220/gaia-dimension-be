...
import { world as world34, system as system37 } from "@minecraft/server";
...
// GaiaDimensions_BP/src/blocks/purifier.js
var Purifier = class extends Machine {
  static get NAME() {
    return "purifier";
  }
  static get INVENTORY_SIZE() {
   ...
   */
  isRunning() {
    return this.inventory.getItem(0) !== void 0;
  }
};
...
// GaiaDimensions_BP/src/blocks/restructurer.js
var Restructurer = class extends Machine {
  static get NAME() {
    return "restructurer";
  }
  static get INVENTORY_SIZE() {
...
var DimensionSystem = class {
  ...
  static handleTeleport(player, sourceDim, targetDimId2, isToGaia) {
    ...
    if (sourceDim.id === targetDimId2) return;
...
world22.afterEvents.entitySpawn.subscribe((event) => {
  ...
  if (entity.typeId === "minecraft:enderman" && DimensionSystem.isInGaia(entity)) {
    if (Math.random() < 0.99) system25.run(() => {
      if (entity.isValid) entity.remove();
    });
  }
});
...
system32.run(() => {
  try {
    the_end = world28.getDimension("minecraft:the_end");
    air = BlockPermutation13.resolve("minecraft:air");
    endstone = BlockPermutation13.resolve("minecraft:end_stone");
    flower = BlockPermutation13.resolve("minecraft:chorus_flower");
    plant = BlockPermutation13.resolve("minecraft:chorus_plant");
    bedrock = BlockPermutation13.resolve("minecraft:bedrock");
    gateway = BlockPermutation13.resolve("minecraft:end_gateway");
    data = DB.getAll();
    Q.run(30);
    world28.sendMessage("TerrainPatching initialized");
  } catch (e) {
...
var MiniChunk = class {
  ...
  set isChecked(value) {
    ...
    if (!chunk && value) {
      ...
      if (data[this.y] && data[this.y][this.x]) {
      ...
      for (let y = -2; y <= 3; y++) {
        for (let x = -radius; x <= radius; x++) {
          for (let z = -radius; z <= radius; z++) {
            ...
            if (checkX >= 1e5 && checkX <= 4e5 && checkZ >= 1e5 && checkZ <= 4e5) {
              Q.push(() => {
...
system32.runInterval(() => {
  ticksPerSecond = 15e4 / (/* @__PURE__ */ new Date() - startTime);
...
// GaiaDimensions_BP/src/blocks/mineral_centrifuge.js
import { world as world31, system as system34, ItemStack as ItemStack10 } from "@minecraft/server";
var MineralCentrifuge = class extends Machine {
  static get NAME() {
    return "mineral_centrifuge";
  }
  static get TIMERS() {
    return {
      spin: { max: 200 },
  ...
  spawnSpinParticles() {
    if (system34.currentTick % 2 !== 0) return;
    const dim = this.block.dimension;
    const center = {
    ...
    const radius = 0.6;
    const speed = 0.5;
    const time = system34.currentTick * speed;
  ...
  completeProcess() {
    ...
    if (consumed) {
      ...
      if (outItem) {
        ...
        this.inventory.setItem(26, new ItemStack10("minecraft:gold_nugget", 1));
      }
      this.block.dimension.playSound("random.levelup", this.block.location);
    }
  }
};
BlockEntity_default.register(MineralCentrifuge);

// GaiaDimensions_BP/src/blocks/arcane_library.js
import { world as world33, system as system36, ItemStack as ItemStack12 } from "@minecraft/server";

// GaiaDimensions_BP/src/API/lib/AdvancedMachine.js
import { world as world32, system as system35, ItemStack as ItemStack11 } from "@minecraft/server";
var MachineStorage = class {
   ...
   */
  async load() {
    const structure = world32.structureManager.get(this.key);
    if (!structure) return [];
    try {
      world32.structureManager.place(this.key, this.dimension, this.storageLoc, { includeEntities: true });
    } catch (e) {
  ...
  async save(items) {
    ...
    if (items.length === 0) {
      if (world32.structureManager.get(this.key)) {
        world32.structureManager.delete(this.key);
      }
      return;
    }
    ...
    world32.structureManager.createFromWorld(this.key, this.dimension, this.storageLoc, this.storageLoc, { saveMode: "World", includeEntities: true });
    if (storageEntity.isValid) storageEntity.remove();
  }
};
var AdvancedMachine = class extends Machine {
  /**
   * @param {Entity} entity 
   * @param {Block} block 
   */
  ...
  renderPage() {
    ...
    for (let i = 0; i < config.count; i++) {
      ...
      if (virtualItem) {
        this.inventory.setItem(physicalSlot, new ItemStack11(virtualItem.typeId, virtualItem.amount));
        this.inventory.setItem(physicalSlot, virtualItem);
      } else {
        this.inventory.setItem(physicalSlot, void 0);
      }
    }
    ...
    for (let i = 0; i < config.count; i++) {
      ...
      if (storedItem && !currentItem) {
        if (this.canTakeItems()) {
          ...
          this.inventory.setItem(physicalSlot, storedItem);
          const player = this.cachedPlayers[0];
          if (player) {
...
// GaiaDimensions_BP/src/blocks/arcane_library.js
var ArcaneLibrary = class extends AdvancedMachine {
  static get NAME() {
    return "arcane_library";
  }
  static get UI_CONFIG() {
    return {
      classicProfile: {
  ...
  depositItem(item) {
    ...
    this.saveStorage();
    this.inventory.setItem(22, void 0);
    this.block.dimension.playSound("random.orb", this.block.location);
    this.renderPage();
  }
};
BlockEntity_default.register(ArcaneLibrary);
...
initializeSkybox();
registerCustomTool();
system37.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
