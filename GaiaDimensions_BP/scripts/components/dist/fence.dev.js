...
exports.updateFenceConnections = updateFenceConnections;
exports.registerFenceComponent = registerFenceComponent;

var _server = require("@minecraft/server");

var _BlockUpdate = require("../systems/BlockUpdate.js");
...
function isConnectable(block) {
  ...
  var typeId = block.typeId; // An expanded list of non-connectable blocks for better accuracy.

  var nonConnectableKeywords = ["snow", "mushroom", "grass", "fern", "flower", "sapling", "vine", "crop", "dead_bush", "leaves", "scaffolding", "sign", "banner", "torch", "lantern", "button", "lever", "invisible", "door", "trapdoor", "leaf_litter"];

  for (var _i = 0, _nonConnectableKeywor = nonConnectableKeywords; _i < _nonConnectableKeywor.length; _i++) {
    var keyword = _nonConnectableKeywor[_i];
...
function updateFenceConnections(block) {
  ...
  try {
    var permutation = block.permutation;
  ...
  } catch (e) {// Suppress errors if the state doesn't exist on the block
  }
}
/**
...
function updateInvisibleBlock(block) {
  if (!block) return;
  var blockAbove = block.above();
  if (!blockAbove) return;
  var typeId = block.typeId;
  var isOpen = typeId.includes("fence_gate") ? block.permutation.getState("gaiadimension:open") : false;
...
function updateNeighborsAt(location, dimension) {
  var x = location.x,
      y = location.y,
      z = location.z;
  var north = dimension.getBlock({
    x: x,
    y: y,
    z: z - 1
  });
  var south = dimension.getBlock({
    x: x,
    y: y,
    z: z + 1
  });
  var east = dimension.getBlock({
    x: x + 1,
    y: y,
    z: z
  });
  var west = dimension.getBlock({
    x: x - 1,
    y: y,
    z: z
  });
 ...
 * This is the original, more precise function.
 * @param {import("@minecraft/server").Vector3} playerFacing The player's view direction vector.
 ...
 * @param {import("@minecraft/server").Block} block The fence gate block.
 */
...
function handleFenceGateInteract(player, block) {
  ...
  var isOpen = block.permutation.getState("gaiadimension:open"); // Only toggle the 'open' state. Do not change the direction.

  var newPermutation = block.permutation.withState("gaiadimension:open", !isOpen);
  block.setPermutation(newPermutation);
  updateInvisibleBlock(block);
  player.playSound(isOpen ? "close.fence_gate" : "open.fence_gate", {
...
function registerFenceComponent(_ref) {
  var blockComponentRegistry = _ref.blockComponentRegistry;
  blockComponentRegistry.registerCustomComponent("gaiadimension:fence", {});
  (0, _BlockUpdate.registerForBlockUpdates)({
    check: function check(block) {
      return block.typeId.includes("fence") && !block.typeId.includes("fence_gate") && !block.typeId.startsWith("minecraft:");
    },
    update: updateFenceConnections
  });
  (0, _event_manager.registerPlaceHandler)({
    check: function check(block) {
      return block.typeId.includes("fence");
    },
    execute: function execute(event) {
      var block = event.block,
          player = event.player; // If the placed block is a custom fence gate, set its initial direction.

      if (block.typeId.includes("fence_gate") && !block.typeId.startsWith("minecraft:")) {
        // Uses the corrected (original) function to get the precise direction.
        var gateDirection = getGateDirectionFromPlayerFacing(player.getViewDirection());
        var newPermutation = block.permutation.withState("minecraft:cardinal_direction", gateDirection);
        block.setPermutation(newPermutation);
  ...
  (0, _event_manager.registerBreakHandler)({
    ...
    execute: function execute(event) {
      ...
      var location = event.block.location; // Remove invisible block above the broken block (if any)

      var blockAbove = dimension.getBlock({
        x: location.x,
        y: location.y + 1,
        z: location.z
      });
      ...
      var blockBelow = dimension.getBlock({
        x: location.x,
        y: location.y - 1,
        z: location.z
      });
      ...
      if (blockBelow && blockBelow.typeId.includes("fence") && !blockBelow.typeId.startsWith("minecraft:")) {
        var fenceTop = dimension.getBlock({
          x: location.x,
          y: location.y,
          z: location.z
        });
  ...
  (0, _event_manager.registerInteractHandler)({
    check: function check(block) {
      return block.typeId.includes("fence") || block.typeId === INVISIBLE_BLOCK_ID;
    },
    execute: function execute(event) {
      var player = event.player,
          block = event.block,
          blockFace = event.blockFace,
          itemStack = event.itemStack; // Handle fence gate opening/closing

      if (block.typeId.includes("fence_gate") && !block.typeId.startsWith("minecraft:")) {
        if (!(player.isSneaking && blockFace === _server.Direction.Up)) {
          event.cancel = true;

          _server.system.run(function () {
            return handleFenceGateInteract(player, block);
          });
      ...
      if (itemStack) {
        ...
        if (block.typeId === INVISIBLE_BLOCK_ID) {
          ...
          if (blockBelow && blockBelow.typeId.includes("fence")) {
            ...
            _server.system.run(function () {
              ...
              if (player.gameMode !== _server.GameMode.creative) {
                var inventory = player.getComponent("inventory");
                var container = inventory.container;
                var item = container.getItem(player.selectedSlot);
        ...
        } else if (blockFace === _server.Direction.Up || player.isSneaking) {
          event.cancel = true;
...
//# sourceMappingURL=fence.dev.js.map
